...
3. **CI 离线全绿**：无 `OPENAI_API_KEY` 的环境下 `cargo test` 全部通过，其中包含至少 3 个走盒带的 Agent 端到端测试。
   > 实现备注：全量 `cargo test` 中 `team::runtime` 两个测试与 `agent::tests::interrupt_listener_cancels_token_when_interrupt_fires` 在 master 上即已失败（与本章无关的既有问题）；本章全部评测测试（含 4 个盒带 e2e 与提交版盒带回放守卫）离线全绿。
